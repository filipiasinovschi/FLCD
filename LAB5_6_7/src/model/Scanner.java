...
public class Scanner {
    ...
    private static final String CODIFICATION_FILE_PATH = "./data/tokens.txt";
    ...
    public void displayPIFReadable(List<Pair<Integer, Integer>> pif) {
        ...
        for (Pair pair : pif) {
            ...
            String codificationTableToken = tokens.get(codificationTableCode);
            if (symbolTablePosition == -1) {
                pifReadable.add(new Pair<>(codificationTableToken + " " + codificationTableCode, " fromCodification "));
            } else {
    ...
    private void tokenizeLine(List<String> tokens) {
        for (int i = 24; i < this.tokens.size(); i++) {
            List<String> tokensCopy = new ArrayList<>(tokens);
            String splitter = "";
            if (this.tokens.get(i).equals("+")) {
                splitter += "\\" + this.tokens.get(i);
            } else if (this.tokens.get(i).equals("*")) {
                splitter += "\\" + this.tokens.get(i);
...
//            System.out.println(tokens);
            for (String part : tokensCopy) {
                ...
                else if (part.contains(this.tokens.get(i)) && part.length() != this.tokens.get(i).length()) {
...
//                        }
    ...
    private void identify(String part) throws IdentifierNameTooLongException, IllegalIdentifierNameException {
        if (tokens.contains(part)) {
            pif.add(new Pair(tokens.indexOf(part), -1));
        } else {
    ...
    private void getCodif() {
        try {
            tokens.addAll(Files.readAllLines(Paths.get(CODIFICATION_FILE_PATH)));
        } catch (IOException e) {
            e.printStackTrace();
        }

        System.out.println("Language codification: " + tokens + '\n');
    }
}